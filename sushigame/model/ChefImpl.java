...
public class ChefImpl implements Chef, BeltObserver {
	...
	public ChefImpl(String name, double starting_balance, ChefsBelt belt) {
		...
		this.weightConsumed = 0.0;
		this.weightSpoiled = 0.0;
		this.belt = belt;
	...
	public void handleBeltEvent(BeltEvent e) {
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				...
				this.weightConsumed += plate.getContents().getTotalWeight();
			...
			Plate plate = ((PlateEvent) e).getPlate();
			if (plate.getChef() == this) {
				this.weightSpoiled += plate.getContents().getTotalWeight();
	...
	@Override
	public double getWeightComsumed() {
		return this.weightConsumed;
	}

	@Override
	public double getWeightSpoiled() {
		return this.weightSpoiled;
	}
}
